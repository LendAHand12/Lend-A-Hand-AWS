...
const LegalPage = ({ match }) => {
  ...
  useEffect(() => {
    (async () => {
      setLoading(true);
      await Posts.getAllPosts(pageNumber, "", category, "")
        .then((response) => {
  ...
  return (
    <section className="text-gray-600 body-font">
      <div className="container px-5 sm:py-10 mx-auto max-w-7x1">
        <div className="flex flex-wrap w-full mb-8">
          <div className="w-full sm:mb-6 lg:mb-0">
        ...
        {category !== "COMMON" && (
          <div className="flex items-center flex-wrap gap-4 sm:mb-10 my-8">
            {[...Array(5)].map((item, i) => (
              <Link
                to={`/user/legal/TIER${i + 1}`}
                key={i}
                className={`flex justify-center items-center hover:underline gradient text-white font-bold ${
                  category === `TIER${i + 1}` ? "border-2 border-gray-700" : ""
                } rounded-full sm:my-6 py-4 px-8 shadow-lg focus:outline-none focus:shadow-outline transform transition hover:scale-105 duration-300 ease-in-out`}
              >
                {t("tier")} {i + 1}
              </Link>
            ))}
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 md:grid-cols-2 gap-4">
          {data.length > 0 &&
            data.map((news, i) => (
              <a
                href={`/user/legal/${category}/${news._id}`}
                key={i}
                ...
                <div className="text-gray-800 text-center p-4 w-full">
                  <div className="flex justify-between items-center gap-4">
                    <div className="flex flex-col items-start">
                      <h3 className="font-semibold text-lg text-left sm:truncate max-w-xs">
